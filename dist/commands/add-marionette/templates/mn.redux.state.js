...
module.exports = createStore(reducer, initialState, compose(applyMiddleware(dispatchLogger), addGetStatePathParameter())); // state reducer
...
function dispatchLogger() {
  return next => action => // console.log(`Dispatch: ${action.type}`);
  next(action);
} // store enhancer
