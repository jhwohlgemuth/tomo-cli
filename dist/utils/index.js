 ...
 */exports.showVersion=showVersion;const install=/*#__PURE__*/function(){var _ref3=(0,_asyncToGenerator2.default)(function*(dependencies=[],options={dev:!1,latest:!1,skipInstall:!1}){const{dev,latest,skipInstall}=options,args=["install"].concat(dependencies.map(str=>{let data;try{data=(0,_npmPackageArg.default)(str)}catch(_unused){data={}}return data}).filter(({name})=>(0,_validateNpmPackageName.default)(name).validForNewPackages).map(({name,fetchSpec})=>{const version=latest?"latest":fetchSpec;return`${name}@${version}`})).concat(dev?"--save-dev":[]);return skipInstall||(yield(0,_execa.default)("npm",args)),args});return function(){return _ref3.apply(this,arguments)}}();exports.install=install;const uninstall=/*#__PURE__*/function(){var _ref4=(0,_asyncToGenerator2.default)(function*(dependencies=[]){const args=["uninstall"].concat(dependencies.filter(name=>(0,_validateNpmPackageName.default)(name).validForNewPackages));return 1===args.length||(yield(0,_execa.default)("npm",args)),args});return function(){return _ref4.apply(this,arguments)}}();/**
