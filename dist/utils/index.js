 ...
 */exports.getIntendedInput=getIntendedInput;const getProjectName=()=>{const a=__dirname,{packageJson:b}=_readPkgUp.default.sync({cwd:a})||{packageJson:{name:"tomo-cli-run"}},{name:c}=b;return c};/**
 ...
 */exports.getProjectName=getProjectName;const debug=/*#__PURE__*/function(){var a=(0,_asyncToGenerator2.default)(function*(a,b="",c={}){const{filename:d}=c,e=d||getProjectName(),f=(0,_path.join)((0,_os.homedir)(),`.${e}`),[g]=new Date().toISOString().split("T"),h=new Date().toLocaleTimeString("en-US",{hour12:!1});try{yield(0,_fsExtra.mkdirp)(f),yield append(`${f}/debug`,`[${`${g} ${h}`}] ${b}${_os.EOL}`),yield append(`${f}/debug`,(0,_common.format)(a)),"string"==typeof a&&0===a.length||(yield append(`${f}/debug`,_os.EOL))}catch(a){/* do nothing */}});return function(){return a.apply(this,arguments)}}();/**
 ...
 *     .commit();
 */function _populateQueue(){return _populateQueue=(0,_asyncToGenerator2.default)(function*({concurrency:a=1,tasks:b=[],dispatch:c=()=>{},options:d={skipInstall:!1}}={}){const{skipInstall:e}=d,f="tomo-cli-run",g=e||(yield(0,_isOnline.default)()),h=assign({},b.filter((0,_complement2.default)(isValidTask)).reduce((a,b)=>assign(a,b),d),{isNotOffline:g}),i=new _pQueue.default({concurrency:a});c({type:"status",payload:{online:g}}),yield debug(h,`Executing <= ${b.filter(isValidTask).length} tasks`,{filename:f});for(const[e,g]of b.filter(isValidTask).filter(isUniqueTask).entries()){const{condition:a,task:b}=g;try{(yield a(h))?(yield debug("",`Executing "${g.text}" task`,{filename:f}),yield i.add(()=>b(h)).then(()=>c({type:"complete",payload:e})).catch(a=>{debug(a,`Error during "${g.text}" task`,{filename:f}),c({type:"error",payload:{index:e,title:"Failed to add task to queue",location:"task",details:g.text}})})):c({type:"skipped",payload:e})}catch(a){debug(a,`Error during "${g.text}" task`,{filename:f}),c({type:"error",payload:{error:a,index:e,title:"Failed to test task conditions",location:"condition",details:g.text}})}}}),_populateQueue.apply(this,arguments)}const BabelConfigModuleEditor=(0,_createModuleEditor.createModuleEditor)("babel.config.js",{plugins:[`'@babel/plugin-transform-runtime'`,`'@babel/plugin-proposal-class-properties'`,`'@babel/plugin-proposal-export-default-from'`,`'@babel/plugin-proposal-optional-chaining'`],presets:[`'@babel/preset-env'`,`'babel-preset-minify'`]});/**
