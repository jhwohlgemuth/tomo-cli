 ...
 */exports.getIntendedInput=getIntendedInput;const debug=/*#__PURE__*/function(){var _ref=(0,_asyncToGenerator2.default)(function*(data,options={}){const{filename="tomo-cli-debug",store,title=""}=options,savepath=(0,_path.join)((0,_os.homedir)(),`.${filename}`),[date]=new Date().toISOString().split("T"),time=new Date().toLocaleTimeString("en-US",{hour12:!1}),timestamp=`${date} ${time}`;try{const previous=store.get("debug")||[];store.set("debug",previous.concat([[`[${timestamp}] ${title}${_os.EOL}`,(0,_common.format)(data)]]))}catch(_){try{yield(0,_fsExtra.mkdirp)(savepath),yield append(`${savepath}/debug`,`[${timestamp}] ${title}${_os.EOL}`),yield append(`${savepath}/debug`,(0,_common.format)(data)),(0,_common.isEmptyString)(data)||(yield append(`${savepath}/debug`,_os.EOL))}catch(_){/* do nothing */}}});return function(){return _ref.apply(this,arguments)}}();/**
 ...
 */exports.showVersion=showVersion;const install=/*#__PURE__*/function(){var _ref3=(0,_asyncToGenerator2.default)(function*(dependencies=[],options={dev:!1,latest:!1,legacy:!1,skipInstall:!1}){const{dev,latest,legacy,skipInstall}=options,args=["install"].concat(dependencies.map(str=>{let data;try{data=(0,_npmPackageArg.default)(str)}catch(_unused){data={}}return data}).filter(({name})=>(0,_validateNpmPackageName.default)(name).validForNewPackages).map(({name,fetchSpec})=>{const version=latest?"latest":fetchSpec;return`${name}@${version}`})).concat(dev?"--save-dev":[]).concat(legacy?"--legacy-peer-deps":[]);return skipInstall||(yield(0,_execa.default)("npm",args)),args});return function(){return _ref3.apply(this,arguments)}}();exports.install=install;const uninstall=/*#__PURE__*/function(){var _ref4=(0,_asyncToGenerator2.default)(function*(dependencies=[]){const args=["uninstall"].concat(dependencies.filter(name=>(0,_validateNpmPackageName.default)(name).validForNewPackages));return 1===args.length||(yield(0,_execa.default)("npm",args)),args});return function(){return _ref4.apply(this,arguments)}}();/**
 ...
 *     .commit();
 */exports.BabelConfigJsonEditor=BabelConfigJsonEditor;const BabelConfigModuleEditor=(0,_createModuleEditor.createModuleEditor)("babel.config.js",{plugins:[`'@babel/plugin-transform-runtime'`,`'@babel/plugin-proposal-class-properties'`,`'@babel/plugin-proposal-export-default-from'`,`'@babel/plugin-proposal-optional-chaining'`],presets:[`'@babel/preset-env'`,`'babel-preset-minify'`]});exports.BabelConfigModuleEditor=BabelConfigModuleEditor;const BsConfigJsonEditor=(0,_createJsonEditor.default)("bsconfig.json",{"bs-dependencies":["reason-react"],"bsc-flags":["-bs-super-errors"],namespace:!0,"package-specs":[{module:"es6","in-source":!0}],"ppx-flags":[],reason:{"react-jsx":3},refmt:3,sources:[{dir:"src",subdirs:!0}],suffix:".bs.js"});/**
 ...
 *     .create()
 *     .commit();
 */exports.BsConfigJsonEditor=BsConfigJsonEditor;const EslintConfigModuleEditor=(0,_createModuleEditor.createModuleEditor)(".eslintrc.js",{env:{es6:!0,jest:!0},extends:[`'omaha-prime-grade'`],parser:`'babel-eslint'`});/**
 ...
 *     .create()
 *     .commit();
 */exports.PackageJsonEditor=PackageJsonEditor;const PostcssConfigEditor=(0,_createModuleEditor.createModuleEditor)("postcss.config.js",{map:!0,parser:`require('postcss-safe-parser')`});/**
